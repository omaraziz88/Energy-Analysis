...
CO2_emissions_df_merged = CO2_emissions_df.merge(
    latlon_df, how='inner', on='Country')
# CO2_emissions_api = CO2_emissions_df_merged.to_json(orient="records")

with open('static/data/countries.geojson', 'r') as f:
    countries = json.load(f)

# with open('static/data/test_co2.json', 'r') as t:
#     CO2_emissions_df_merged = json.load(t)
...
for feat in countries['features']:
    ...
    value_2010 = "NA"
    value_2017 = "NA"
    for i in range(len(CO2_emissions_df_merged)):
        dict = CO2_emissions_df_merged[i]
...
CO2_emissions_api = countries

# # Write result to a new file
# with open('static/data/countries_new.geojson', 'w') as f:
